...
class UglyBrain(Brain):
	...
	def update(self):
		...
		stoppingContant = 30
		acceletationConstant = 4
		maxSpeed = 3
		horizontalAccelerationConstant = 0.02
		horizontalAccelerationConstant2 = 20
		self.car.accelerate(-stoppingContant*frontUrgency)
		if(self.car.speed < maxSpeed):
			self.car.accelerate(acceletationConstant)

		if(self.front_center_sensor.is_obstacle_goal):
			self.car.accelerate(2)
		self.car.turn(-self.front_right_sensor.distance * horizontalAccelerationConstant)
		self.car.turn(self.front_left_sensor.distance * horizontalAccelerationConstant)

	

		if(self.front_left_sensor.distance < 10):
			self.car.turn(horizontalAccelerationConstant2)
			self.car.accelerate(2)
